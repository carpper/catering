	...
	<div class="main-content" :style='{"padding":"20px 30px 20px 30px","margin":"130px 0 0 20px","fontSize":"15px","background":"#F2F8FF"}'>
		...
		<template v-if="showFlag">
          ...
          <div :style='{"margin":"0 10px 0 0","display":"flex"}' v-if="isAdmin()">
            ...
            <el-input v-model="searchForm.qiyemingcheng" placeholder="企业名称" @keydown.enter.native="search()" clearable></el-input>
					...
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">地区</label>
						<el-input v-model="searchForm.diqu" placeholder="地区" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select" label="求职状态" prop="qiuzhizhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">求职状态</label>
						<el-select clearable v-model="searchForm.qiuzhizhuangtai" placeholder="请选择求职状态" >
							<el-option v-for="(item,index) in qiuzhizhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('toudixinxi','热门岗位')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						热门岗位
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="gerenjianli" label="个人简历">
						<template slot-scope="scope">
          ...
          <el-table-column :resizable='true' :sortable='true' prop="shhf" label="审核回复" show-overflow-tooltip>
            <template slot-scope="scope">
              <div style="white-space: nowrap; font-size: 18px; color: #000000;">{{scope.row.shhf}}</div>
            </template>
          </el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('toudixinxi','邀请面试')" @click="mianshixinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','qiuzhizhuangtai','已邀请','已邀请,待通知'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								邀请面试
							</el-button>
							<el-button class="btn8" v-if="isAuth('toudixinxi','评价')" @click="qiyepingjiaCrossAddOrUpdateHandler(scope.row,'cross','否','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								评价
							</el-button>
		...
		<qiyepingjia-cross-add-or-update v-if="qiyepingjiaCrossAddOrUpdateFlag" :parent="this" ref="qiyepingjiaCrossaddOrUpdate"></qiyepingjia-cross-add-or-update>
    ...
    <el-dialog :title="this.batchIds.length>1?'批量审核':'审核'" :visible.sync="sfshBatchVisiable" width="50%">
      <el-form ref="shBatchForm" :model="shBatchForm" :rules="shRules" label-width="80px">
        ...
        <el-form-item label="内容" prop="shhf">
          ...
          <el-input type="textarea" :rows="8" v-model="shBatchForm.shhf" class="review-textarea"></el-input>
        </el-form-item>
      </el-form>
...
<script>
	...
	import qiyepingjiaCrossAddOrUpdate from "../qiyepingjia/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				qiyepingjiaCrossAddOrUpdateFlag: false,
      ...
      isAdmin() {
        ...
        return role == 'admin' || role == '管理员';
			...
			qiyepingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.qiyepingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','toudixinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.qiyepingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.qiyepingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				if(this.searchForm.qiuzhizhuangtai!='' && this.searchForm.qiuzhizhuangtai!=undefined){
					params['qiuzhizhuangtai'] = this.searchForm.qiuzhizhuangtai
				}
