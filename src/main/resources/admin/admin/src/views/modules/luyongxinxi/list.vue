			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 30px  10px 30px","alignSelf":"center","borderRadius":"30px 30px 0 0 ","background":"#fff","display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">录用时间</label>
						<el-date-picker
							v-model="searchForm.luyongshijianStart"
							type="date"
							placeholder="开始日期"
							value-format="yyyy-MM-dd">
						</el-date-picker>
						<span :style='{"margin":"0 10px","lineHeight":"40px"}'>至</span>
						<el-date-picker
							v-model="searchForm.luyongshijianEnd"
							type="date"
							placeholder="结束日期"
							value-format="yyyy-MM-dd">
						</el-date-picker>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('luyongxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('luyongxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('luyongxinxi','聘用岗位')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('luyongxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('luyongxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('luyongxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							var option = {};
							let titleObj = JSON.parse(JSON.stringify(this.line.title))
							titleObj.text = '聘用岗位'
							
							const legendObj = JSON.parse(JSON.stringify(this.line.legend))
							let tooltipObj = { trigger: 'item',formatter: '{b} : {c}'}
							tooltipObj = Object.assign({}, tooltipObj, this.line.tooltip?this.line.tooltip:{})
							let xAxisObj = JSON.parse(JSON.stringify(this.line.xAxis))
							xAxisObj.type = 'category'
							xAxisObj.boundaryGap = false
							xAxisObj.data = xAxis
							
							let yAxisObj = JSON.parse(JSON.stringify(this.line.yAxis))
							yAxisObj.type = 'value'
							const gridObj = JSON.parse(JSON.stringify(this.line.grid))
							let seriesObj = {
								data: yAxis,
								type: 'line',
								smooth: true
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.luyongshijianStart!='' && this.searchForm.luyongshijianStart!=undefined){
					params['luyongshijian_start'] = this.searchForm.luyongshijianStart
				}
				if(this.searchForm.luyongshijianEnd!='' && this.searchForm.luyongshijianEnd!=undefined){
					params['luyongshijian_end'] = this.searchForm.luyongshijianEnd
				}
