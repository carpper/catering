...
window.onerror = function(msg, url, line, col, error) {
  if (msg && msg.indexOf && msg.indexOf('AMap') !== -1) {
    console.warn('高德地图加载失败，部分地图功能可能不可用');
...
Vue.http.interceptors.push(function(request, next) {
	// 每次请求前从 localStorage 获取最新的 Token
	request.headers.set('Token', localStorage.getItem('frontToken'));
	next((response) => {
