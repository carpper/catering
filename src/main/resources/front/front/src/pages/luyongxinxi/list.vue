	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('luyongxinxi','新增')" type="primary" @click="add('/index/luyongxinxiAdd')">
			...
			<div class="list">
				...
				<div class="list2 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" class="list-item animation-box">
						...
						<div class="item-info">
							<div class="name">{{item.luyongbianhao}}</div>
							<div class="name">{{item.qiyemingcheng}}</div>
							<div class="name">{{item.luyongshijian}}</div>
							<div class="time_item">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`luyongxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/luyongxinxiDetail', query: params});
			},
