<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="160px"
			>
			<el-form-item class="add-item" label="录用编号" prop="luyongbianhao">
				<el-input v-model="ruleForm.luyongbianhao" placeholder="录用编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="录用岗位" prop="gangweileixing">
				<el-input v-model="ruleForm.gangweileixing" 
					placeholder="录用岗位" clearable :disabled=" false  ||ro.gangweileixing"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="录用时间" prop="luyongshijian">
				<el-date-picker
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.luyongshijian = this.getCurDateTime()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			// 多级联动参数
			info() {
				this.$http.get(`luyongxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 提交
			async onSubmit() {
				if(this.ruleForm.luyongbianhao){
					this.ruleForm.luyongbianhao = String(this.ruleForm.luyongbianhao)
				}
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`luyongxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
