<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="求职岗位" prop="gangweileixing">
				<el-input v-model="ruleForm.gangweileixing" 
					placeholder="求职岗位" clearable :disabled=" false  ||ro.gangweileixing"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="个人经历" prop="gerenjingli">
				<el-input v-model="ruleForm.gerenjingli" 
					placeholder="个人经历" clearable :disabled=" false  ||ro.gerenjingli"></el-input>
			</el-form-item>
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`qiyepingjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
