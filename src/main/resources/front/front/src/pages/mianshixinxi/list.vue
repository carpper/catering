				...
				<div class="list2 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" class="list-item animation-box">
						...
						<div class="item-info">
							...
							<div class="name" v-if="item.mianshixingshi">面试方式：{{item.mianshixingshi}}</div>
							<div class="name" v-if="item.mianshishijian">面试时间：{{item.mianshishijian}}</div>
							<div class="name" v-if="item.mianshididian">地点/链接：{{item.mianshididian}}</div>
							<div class="time_item">
...
// 在 created() 方法中修改 mianshijieguoOptions 的初始化
    async created() {
      ...
      // 修改这行，添加"未确认"选项
      this.mianshijieguoOptions = '已录用,未录用,未确认'.split(',');
      ...
      this.shifoucanchuOptions = '未确认'.split(',');
      this.selectOptionsList.push({name:'是否参加',list:this.shifoucanchuOptions,tableName: 'shifoucanchu',check: -1})
...
,
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.mianshijieguo != '') searchWhere.mianshijieguo = this.formSearch.mianshijieguo;
        // 添加这一行处理 shifoucanchu 筛选
        if (this.formSearch.shifoucanchu != '') searchWhere.shifoucanchu = this.formSearch.shifoucanchu;
