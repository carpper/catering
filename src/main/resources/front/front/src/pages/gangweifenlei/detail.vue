	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gangweifenlei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('gangweifenlei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('gangweifenlei','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/gangweifenlei', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此岗位分类？') .then(_ => {
